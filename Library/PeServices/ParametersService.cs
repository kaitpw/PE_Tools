...
public class ParametersService {
    public async Task<IEnumerable<dynamic>> GetHubs(string token) {
        ...
        if (response.IsSuccessStatusCode) {
            ...
            foreach (var field in dynamicObject.data)
                hubs.Add(new Tuple<string, string>(Convert.ToString(field.attributes.name),
                    Convert.ToString(field.id)));
        }
    ...
    public async Task<IEnumerable<dynamic>> GetAccounts(string token, string hubname) {
        ...
        if (response.IsSuccessStatusCode) {
            var dynamicObject = JsonConvert.DeserializeObject<dynamic>(await response.Content.ReadAsStringAsync());
            foreach (var field in dynamicObject.data)
                accounts.Add(new Tuple<string, string>(Convert.ToString(field.attributes.name),
                    Convert.ToString(field.id)));
        }
    ...
    public async Task<List<ParametersViewModel>> GetParameters(string accountId,
        string groupId,
        string collectionId,
        string token) {
        var parameters = new List<ParametersViewModel>();
                ...
                foreach (var field in dynamicObject.results) {
                    ...
                    foreach (var meta in field.metadata) {
                        if (meta.id == "instanceTypeAssociation")
                            ...
                            foreach (var cat in meta.value) {
                                string catNames = cat.id;
                                contentResults.Add(catNames.Replace("autodesk.revit.category.local:", "")
                                    .Replace("autodesk.revit.category.family:", "").Replace("-1.0.0", ""));
                            }
                    ...
                    if (!metaType.Equals("NONE"))
                        parameters.Add(new ParametersViewModel {
                            ...
                            TypeOrInstance = metaType,
                            Category = category,
                            Id = Convert.ToString(field.id)
                        });
                }
            }
...
public class ParametersViewModel {
    ...
    [JsonProperty("Type / Instance")] public string TypeOrInstance { get; set; }
