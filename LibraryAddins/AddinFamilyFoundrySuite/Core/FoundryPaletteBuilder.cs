...
public class FoundryPaletteBuilder<TProfile> where TProfile : BaseProfileSettings, new() {
    ...
    /// <summary>
    ///     Helper to execute the standard processing flow.
    ///     Commands can call this from their action handlers.
    /// </summary>
    public void ExecuteProcessing(FoundryContext<TProfile> ctx) {
        ...
        var queue = this._queueBuilder(profile, apsParamData);
        ...
        this._postProcess?.Invoke(ctx, processedFamilyNames);
