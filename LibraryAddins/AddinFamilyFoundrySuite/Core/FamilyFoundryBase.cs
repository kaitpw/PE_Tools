...
public abstract class FamilyFoundryBase<TSettings, TProfile> : IFamilyFoundry<TSettings, TProfile>
    ...
    public void InitAndFetch() {
        ...
        // test if the cache file exists, if not throw error to prompt user to run command to generate cache
        try {
            var _ = this._settings.GetAPSParams();
        } catch (FileNotFoundException) {
            throw new InvalidOperationException(
                $"This Family Foundry command requires cached parameters data, but no cache file exists. " +
