        ...
        foreach (var param in paramsWithFormulas) {
            ...
            try {
                ...
                var logMsg = sourceName == paramName
                    ? $"{paramName}: unwrapped constant formula"
                    : $"{paramName}: resolved from {sourceName}";

                logs.Add(new LogEntry { Item = logMsg });
            } catch (Exception ex) {
                logs.Add(new LogEntry { Item = paramName, Error = ex.Message });
            }
        }
